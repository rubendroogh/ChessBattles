require(['./chess.js'], function (Chess) {
    ...
    function defineEvents() {
        document.getElementById("activatebot2").addEventListener("click", _ => {doRequest()});
        console.log('Events defined bot 2!');
    }

    function doRequest(move) {
        const parameterData = {
            secret: secret,
            move: move
        }
        var URL = endpoint + new URLSearchParams(parameterData).toString();
        ...
        .then(responseJSON => {
            if (responseJSON.success == 1 && responseJSON.color == 'b'){
                setTimeout(function () {
